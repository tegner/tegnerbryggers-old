...
class Menu extends React.Component {
  ...
  shouldComponentUpdate (nextProps, nextState) {
    return (this.props.isLoading !== nextProps.isLoading) || (this.state.closed !== nextState.closed)
  }

  render () {
    const { isLoading, posts } = this.props
    console.log('Menu.render this.props:', this.props)
    console.log('Menu.render this.state.closed: ', this.state.closed)
    return <div>
      {isLoading && 'Loading...'}
      {!isLoading && (
        <nav id='menu' className={this.state.closed ? 'menu' : 'menu menu--opened'}>
          ...
          <div className='menu-handle' onClick={this.MenuToggler}>
            <svg id='Layer_1' data-name='Layer 1' xmlns='http://www.w3.org/2000/svg' viewBox='0 0 137.28 226.82'>
              <title>angle</title>
